    ...
    if input_file is None:
        ...
        if input_file.endswith('.csv'):
            ...
            with open(input_file, 'r', encoding='utf-8',
                ...
                text = txt_file.read()
                for input_text in text.split("\n")[0:args.batch]:
                    input_ids = tokenizer.encode(
                    input_text,
                    add_special_tokens=add_special_tokens,
                    truncation=False,
                    max_length=max_input_length)[0:max_input_length]
                    batch_input_ids.append(input_ids[0:max_input_length])
        else:
    ...
    if output_csv is None and output_npy is None:
        for batch_idx in range(batch_size):
            ...
            for beam in range(num_beams):
                ...
                token_num = len(outputs) + token_num
                #print(f'Output tokens: {len(outputs)}')
                #print(f'Output tokens: {token_num}')
                input_array = []
                output_array = []
                for k in inputs:
                    input_array.append(tokenizer.decode([k]))
                for k in outputs:
                    output_array.append(tokenizer.decode([k]))
...
def main(args):
    tensorrt_llm.profiler.start("load")
    ...
    tensorrt_llm.profiler.stop("load")
    print(f"\nLoading model latency: {tensorrt_llm.profiler.elapsed_time_in_sec('load')} sec")

    tensorrt_llm.profiler.start("first")
    with torch.no_grad():
    ...
    tensorrt_llm.profiler.stop("first")
    
    if runtime_rank == 0:
        print(f"First inference latency: {tensorrt_llm.profiler.elapsed_time_in_sec('first')} sec")
    ...
    if args.run_profiling:
        ...
        print(f"Output Max Length (Config): {args.max_output_len}")
        txt_file = open(args.input_file, 'r', encoding='utf-8',errors='replace')
        text = txt_file.read()
        ...
        for _ in range(args.iterations):
            with torch.no_grad():
                batch_input_ids = []
                for input_text in text_list[0:args.batch]:
                    input_ids = tokenizer.encode(input_text,add_special_tokens=True, truncation=False,max_length=args.max_input_length)
                    batch_input_ids.append(input_ids[0:args.max_input_length])
        ...
        if runtime_rank == 0 : 
            ...
            token_len_max = torch.max(outputs['sequence_lengths'])
            # for i in range(len(output_ids)):
            #     print(f"\nText {i} : Length : {len(output_ids[i][0])}")
            #     print(f"\nOutput IDs : {output_ids[i][0]}")
            print(f"Sequence Length Max in the Batch: {token_len_max}")
            print(f"Batch_size: {len(batch_input_ids)}, avg latency of {args.iterations} iterations: : {tensorrt_llm.profiler.elapsed_time_in_sec('tmp') / args.iterations} sec")
            print(f"Totel tokens of {args.iterations} iterations is: {token_len_max * len(outputs['sequence_lengths']) * args.iterations}")
            print(f"Throughput : {token_len_max * len(outputs['sequence_lengths']) * args.iterations / tensorrt_llm.profiler.elapsed_time_in_sec('tmp')} tokens / sec")
